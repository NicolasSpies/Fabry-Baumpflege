...
function mapPathToTargetLang(currentPath, currentLang, targetLang) {
    ...
    for (const [key, path] of Object.entries(currentRoutes)) {
        if (path === currentPath && !path.includes(':')) {
...
export function LanguageProvider({ children }) {
    ...
    // When set, setLanguage delegates to it instead of navigating directly.
    // ReferenceDetail uses this to resolve pll_translations before navigating.
    ...
    return (
        <LanguageContext.Provider value={{ language, setLanguage, t, registerDetailSwitch, unregisterDetailSwitch }}>
            {children}
