...
const ReferenceCard = ({ 
    ...
    return (
        <Link
            ...
            className={`group relative rounded-2xl block shadow-md md:shadow-lg lg:hover:shadow-2xl lg:hover:-translate-y-1 transition-all duration-500 overflow-hidden ${animateEntry ? 'animate-entrance' : ''}`}
            style={animateEntry ? { animationDelay: `${staggerIndex * 0.1}s` } : {}}
        >
            <div className={`relative w-full overflow-hidden aspect-square bg-slate-100 dark:bg-slate-900`}>
                <CmsImage
                    image={props.thumbnailImage}
                    alt={props.title}
                    size="480"
                    className="w-full h-full object-cover transition-transform duration-700 lg:group-hover:scale-110"
                    sizes={props.sizes || "(max-width: 768px) 90vw, (max-width: 1024px) 45vw, 33vw"}
                ...
                <div
                    className={`absolute inset-0 flex flex-col justify-end p-6 md:p-8 text-white transition-all duration-500
                        ...
                        /* Mobile + Tablet: Static visible (touch devices have no hover) */
                        opacity-100
                        /* Desktop (lg+): Hidden by default, Visible on hover */
                        lg:opacity-0 lg:group-hover:opacity-100
                    `}
                >
                    <div className="mb-2 md:mb-3 flex flex-wrap gap-x-3 gap-y-1 transform translate-y-0 lg:translate-y-4 lg:group-hover:translate-y-0 transition-transform duration-500 delay-100">
                    ...
                    <h3 className="font-serif text-xl md:text-2xl mb-1 transform translate-y-0 lg:translate-y-4 lg:group-hover:translate-y-0 transition-transform duration-500 delay-200 leading-tight">
                        {renderCmsInline(props.title)}
                    </h3>
                    
                    <p className="text-[12px] md:text-xs line-clamp-2 opacity-90 lg:opacity-0 lg:group-hover:opacity-100 transform translate-y-0 lg:translate-y-4 lg:group-hover:translate-y-0 transition-all duration-500 delay-300 mb-4 lg:mb-0">
                        {renderCmsInline(props.description)}
                    </p>
                    
                    <div className="mt-2 md:mt-4 transform translate-y-0 lg:translate-y-2 lg:group-hover:translate-y-0 transition-transform duration-500 delay-400">
